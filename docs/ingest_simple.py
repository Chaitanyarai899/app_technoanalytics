...
"""
=============================================================================
MODERN RASTER INGESTION PIPELINE - Simplified Version
Proceso completo: TIF → COG → GCS → Supabase (sin GDAL por ahora)
=============================================================================
"""
...
class RasterIngestionPipeline:
    ...
    def check_file_exists_in_gcs(self, cog_filename: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"No se pudo verificar archivo en GCS (gsutil no disponible): {e}")
            return False
    
    def convert_to_cog(self, input_path: str, output_path: str) -> bool:
        """Convertir TIF a COG usando GDAL"""
        if not RASTERIO_AVAILABLE:
            logger.warning("⚠️  Conversión COG no disponible sin GDAL/rasterio")
        ...
        try:
            ...
            with rasterio.open(input_path) as src:
                ...
                with rasterio.open(output_path, 'w', **profile) as dst:
                    ...
                    dst.update_tags(ns='gdal', **dst.tags(ns='gdal'))
    ...
    def update_database(self, metadata: Dict) -> bool:
        ...
        try:
            ...
            if existing:
                ...
                data = response.json()
                if data and len(data) > 0:
                    logger.info(f"✅ Producto creado en DB: {data[0]['id']}")
                else:
    ...
    def process_file_complete(self, file_path: str, force: bool = False) -> bool:
        ...
        logger.info(f"🔄 Procesando (COG completo): {file_path.name}")
        ...
        try:
            ...
            logger.info(f"🌐 URL pública: https://ta-titiler-service-1087100331392.us-central1.run.app/cog/tiles/{complete_metadata.get('id', 'unknown')}/{{z}}/{{x}}/{{y}}.png")
            return True
            ...
            return False
        """Procesar archivo sin conversión COG (solo metadata y DB)"""
        file_path = Path(file_path)
        ...
        logger.info(f"🔄 Procesando (modo simple): {file_path.name}")
        ...
        try:
            # Crear metadata completa (simulada por ahora)
            complete_metadata = {
                **file_metadata,
                'original_filename': file_path.name,
                'cog_filename': cog_filename,
                'cog_url': f'gs://{self.gcs_bucket}/{gcs_path}',
                'file_size_mb': round(os.path.getsize(file_path) / (1024*1024), 2),
                'processing_status': 'pending',  # Cambiar a pending por ahora
                'processing_log': f"Procesamiento simulado: {datetime.now()}",
                'colormap': self.product_colormaps.get(product_type),
                'rescale_min': self.rescale_defaults.get(product_type, (None, None))[0],
                'rescale_max': self.rescale_defaults.get(product_type, (None, None))[1],
                'epsg': 4326,  # Por defecto
                'bands': 1     # Por defecto
            }
            ...
            logger.info(f"✅ Metadata procesada: {file_path.name}")
            return True
    ...
    def process_directory(self, directory: str, force: bool = False, simple: bool = False) -> Dict:
        ...
        logger.info(f"🔧 Modo: {'Simple (metadata)' if simple else 'Completo (COG + upload)'}")
        ...
        for tif_file in tif_files:
            try:
                if simple:
                    success = self.process_file_simple(tif_file, force)
                else:
                    success = self.process_file_complete(tif_file, force)
...
def main():
    ...
    parser.add_argument('--simple', action='store_true', help='Modo simple (sin conversión COG)')
    ...
    if input_path.is_file():
        ...
        if args.simple:
            success = pipeline.process_file_simple(args.input, args.force)
        else:
            success = pipeline.process_file_complete(args.input, args.force)
        sys.exit(0 if success else 1)
        
    elif input_path.is_dir():
        # Procesar directorio
        results = pipeline.process_directory(args.input, args.force, args.simple)
